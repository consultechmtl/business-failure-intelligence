...
class LoadSqliteTests(unittest.TestCase):
    ...
    def test_loads_curated_corpus_and_joins_assertions_to_companies_and_sources(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("Loaded 148 rows", result.stdout)
            ...
            with sqlite3.connect(database_path) as connection:
                ...
                joined_count = connection.execute(
                    """
                    SELECT COUNT(*)
                    FROM cause_assertions AS assertion
                    JOIN companies AS company ON company.company_id = assertion.company_id
                    JOIN sources AS source ON source.source_id = assertion.source_id
                    """
                ).fetchone()[0]
                self.assertEqual(assertion_count, 37)
